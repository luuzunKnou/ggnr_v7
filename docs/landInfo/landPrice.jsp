...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		var url = "${pageContext.request.contextPath}/vworldIndvdLandPriceAttr.api";
		
		var domain = $(location).attr('protocol') + '//' + $(location).attr('host');
		
		if(document.location.href.indexOf("https") != -1){
			url = "${pageContext.request.contextPath}/vworldIndvdLandPriceAttr_https.api";		
 		};
		...
		xhr.onreadystatechange = function () {
		    if (this.readyState == 4) {
	 			...
	 			var totalCount=0;
	 			if(result.indvdLandPrices){
	 				totalCount=result.indvdLandPrices.totalCount;
	 			}else {
		        ...
		        var dataArray =getReduceDuplicateProcessedData(fieldsToKeep,result.indvdLandPrices.field, "pblntfDe");
		        
		        appendStr += '<div class="source_div"><img src="'+getContextPath()+'/images/icon/info_source.png">vworld 디지털 트윈국도 국가중점데이터</div>';
 				for(var i=0;i<dataArray.length;i++){ 
 	 				appendStr += '<table>'
 					appendStr += '	<tr>'
	 				appendStr += '		<th>공시일자</th>'
	 				appendStr += '		<td>'+dataArray[i].pblntfDe+'</td>'
	 				appendStr += '		<th>공시지가(원/㎡)</th>'
