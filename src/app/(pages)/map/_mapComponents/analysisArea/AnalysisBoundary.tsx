...
import { ANALYSIS_SIGUNGU_BOUNDARY_STYLE } from './analysisArea.style';
import { useMapContext } from '../MapContext';

const EMPTY_REGION: AnalysisRegion = { sido: '', sigungu: '' };
const REGION_TIMEOUT_MS = 12_000;

/** 푸터 설정 기준 시·군구 — 행정경계 선택 기본값 */
function useAnalysisRegion(): AnalysisRegion {
  const [region, setRegion] = useState<AnalysisRegion>(EMPTY_REGION);
...
export function useParcelAnalysisSigunguBoundary(active: boolean) {
  ...
  useEffect(() => {
    ...
    const layer = new VectorLayer({ source, style: ANALYSIS_SIGUNGU_BOUNDARY_STYLE, zIndex: BOUNDARY_LAYER_Z });
...
export function ParcelAnalysisBoundaryPicker({
  ...
  const { sido, sigungu } = useAnalysisRegion();
