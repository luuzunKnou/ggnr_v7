...
import { getRowKey } from "../standard/defineLayerRowUtils";
import {
...
import { ServiceFilePdfThumb } from "../standard/ServiceFilePdfThumb";
import {
...
export function RiverBasicPlanDetailPanel({ tab, riverName, onClose }: Props) {
  ...
  const fitMapToExtent3857 = useCallback(
    (ext: [number, number, number, number] | null | undefined) => {
      ...
      if (width < 1 && height < 1) {
        view.animate({ center: [xmin, ymin], zoom: 20, duration: 500 });
      } else {
    ...
    <div className="flex flex-col min-h-0 h-full bg-white border-l border-slate-200">
      ...
      <div className="flex-1 min-h-0 overflow-auto">
        {indexViewMode ? (
                            ...
                            <div className="flex items-stretch min-h-[30px]">
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  fitMapToExtent3857(
                                    ok ? ext : (indexBundle.index?.extent3857 ?? null)
                                  );
                                }}
                              >
                                <span
          ...
          <p className="text-sm text-slate-500 px-4 py-4">속성정보 불러오는 중...</p>
        ) : !detail || detailEntries.length === 0 ? (
          <div className="p-3 space-y-2">
            <p className="flex flex-wrap items-center gap-1.5 text-xs font-normal text-slate-800">
              <span
                className="inline-flex shrink-0 text-emerald-600"
                title="지도의 하천색인도를 클릭하면 색인도·시설 정보를 볼 수 있습니다."
                aria-label="도움말: 지도의 하천색인도를 클릭하면 색인도·시설 정보를 볼 수 있습니다."
              >
                <CircleCheckBig className="h-3.5 w-3.5" aria-hidden />
              </span>
              <span className="rounded-sm text-gray-600 px-0 py-0.5 [box-decoration-break:clone]">
                지도에서 하천색인도를 선택해주세요.
              </span>
            </p>
            <p className="text-sm text-slate-500">속성정보가 없습니다.</p>
          </div>
        ) : (
          <div className="p-3">
            <p className="mb-2 flex flex-wrap items-center gap-1.5 text-xs font-normal text-slate-800">
              <span
                className="inline-flex shrink-0 text-emerald-600"
                title="지도의 하천색인도를 클릭하면 색인도·시설 정보를 볼 수 있습니다."
                aria-label="도움말: 지도의 하천색인도를 클릭하면 색인도·시설 정보를 볼 수 있습니다."
