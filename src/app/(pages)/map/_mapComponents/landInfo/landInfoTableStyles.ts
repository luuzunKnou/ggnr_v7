...
/** 항목명(라벨) — 그리드·상세표 행 헤더 */
export const LAND_INFO_LABEL_CELL = `${PANEL_DETAIL_LABEL_CELL} text-left align-middle border border-border break-keep`;

/** 값 셀 — 상세표 */
export const LAND_INFO_VALUE_CELL = `${PANEL_DETAIL_VALUE_CELL} text-left align-middle border border-border`;
...
/** 목록 표 — 짝수 행 배경 (필지 토지이용계획과 동일) */
export const LAND_INFO_LIST_ROW_ODD = 'odd:bg-background even:bg-muted/40';

/** 2열 그리드(토지기본정보 등) */
export const LAND_INFO_FIELD_GRID =
  'grid min-w-0 grid-cols-[minmax(4.5rem,5.25rem)_minmax(0,1fr)_minmax(4.5rem,5.25rem)_minmax(0,1fr)] overflow-x-auto rounded border border-border text-[11px]';

/** 그리드 라벨 칸 */
export const LAND_INFO_GRID_LABEL = `min-w-0 border-b border-r border-border ${PANEL_DETAIL_LABEL_CELL}`;

/** 그리드 값 칸 */
export const LAND_INFO_GRID_VALUE = `min-w-0 border-b border-border ${PANEL_DETAIL_VALUE_CELL}`;
