...
/** WMS 심볼 레이어 식별 — 보이는 벡터 오버레이 없이 지도 클릭 → 상세 선택 */
export function useRadiationShelterMapClick({
  mapReady,
  panelOpen,
  items,
  onSelectDetailId,
  ...
  useEffect(() => {
    ...
    const handleClick = async (evt: MapBrowserEvent<PointerEvent>) => {
      ...
      if (visRef.current[RADIATION_SHELTER_GEO_TABLE] !== true) return;
      ...
      try {
        const res = await call('', 'POST', {
          ...
          params: {
            ...
            y,
            buffer: bufferMeters,
            tables: [RADIATION_SHELTER_GEO_TABLE],
            schema: 'layer',
          },
        });
        ...
        const layerHit = results.find(
          (r: { tableName?: string }) =>
            String(r?.tableName ?? '')
              .trim()
              .toLowerCase() === RADIATION_SHELTER_GEO_TABLE
