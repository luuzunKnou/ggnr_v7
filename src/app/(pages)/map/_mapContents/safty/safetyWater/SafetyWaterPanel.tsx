...
function TimeChipGroup({
  ...
  return (
    <div className="inline-flex rounded border border-border bg-card p-0.5" role="group">
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden opacity-[0.98]" aria-label="침수 홍수 현황">
      <div className="relative shrink-0 border-b border-border bg-gradient-to-b from-primary/5 to-background">
        <div className="flex items-start justify-between gap-3 px-4 py-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 cursor-pointer rounded p-1 text-muted-foreground/70 transition-colors hover:bg-muted hover:text-muted-foreground"
            title="닫기"
            aria-label="닫기"
          >
            <X className="h-4 w-4" />
        ...
        <div className="flex h-10 w-full items-center gap-2 border-t border-border/80 px-4">
          <span className="text-[11px] leading-none text-muted-foreground">마지막 갱신</span>
          <span className="text-[11px] font-medium leading-none tabular-nums text-foreground/90">
          ...
          <button
            ...
            className={cn(
              ...
              refreshing || loading ? 'cursor-wait text-muted-foreground/70' : 'text-primary hover:text-primary/80'
            )}
            title="현황 새로고침"
            aria-label="현황 새로고침"
          >
      ...
      <div className="flex shrink-0 flex-col gap-3 overflow-y-auto bg-muted/30 p-3">
          ...
          <div
            className={cn(
              ...
              (uiError ?? obsError)!.errorClass === 'provider'
                ...
                : 'border-red-400/60 bg-destructive/10 text-red-800 dark:text-red-300'
            )}
            role="alert"
          >
        ...
        >
          <div className="flex items-start justify-between gap-2 border-b border-border/60 pb-2">
            <div className="flex min-w-0 items-start gap-2">
              <Umbrella
                ...
                aria-hidden
              />
              <h3 className="min-w-0 break-keep text-[12px] font-semibold leading-snug text-foreground">
                현재 강수량
              </h3>
            </div>
            <div className="flex shrink-0 items-center gap-2">
              ...
              {rainIsPaired ? (
                <span className="max-w-[9rem] text-right text-[10px] leading-snug text-muted-foreground">
                  가장 가까운 강수 관측소 값
                </span>
              ) : null}
              <span
          ...
          {obsLoading ? (
            <div
              className="absolute inset-0 z-10 flex items-center justify-center rounded-[5px] bg-background/70 backdrop-blur-sm"
              aria-hidden
            >
          ...
          <div className="mt-2 space-y-1.5 text-[11px]">
            {rainNoData && !obsLoading ? (
              <p className="text-[11px] text-muted-foreground">검색된 자료가 없습니다.</p>
            ) : (
              <dl className="space-y-1.5">
                <div className="flex justify-between gap-2">
                  <dt className="min-w-0 truncate text-muted-foreground">{rainLabel}</dt>
                  <dd className="shrink-0 font-medium tabular-nums text-foreground">
                    {rainObs?.value != null ? `${rainObs.value.toFixed(1)} mm` : '— mm'}
                  </dd>
                </div>
                <div className="flex justify-between gap-2">
                  <dt className="text-muted-foreground">데이터 기준 일시</dt>
                  <dd className="tabular-nums text-muted-foreground">{formatYmdhm(rainObs?.observedAt, timeType)}</dd>
                </div>
              </dl>
            )}
        ...
        >
          ...
          <div className="mt-2 space-y-1.5 text-[11px]">
            {waterNoData && !obsLoading ? (
              ...
              <dl className="space-y-1.5">
                <div className="flex justify-between gap-2">
                  <dt className="min-w-0 truncate text-muted-foreground">{waterLabel}</dt>
                  <dd className="shrink-0 font-medium tabular-nums text-foreground">
                ...
                <div className="flex justify-between gap-2">
                  <dt className="text-muted-foreground">데이터 기준 일시</dt>
                  <dd className="tabular-nums text-muted-foreground">{formatYmdhm(waterObs?.observedAt, timeType)}</dd>
                </div>
              </dl>
            )}
            <p className="text-[10px] text-muted-foreground/70">정보제공: 한강 홍수통제소</p>
        ...
        <section className="hidden rounded-[5px] border border-border/90 bg-card p-3 shadow-sm" aria-hidden>
          <div className="flex flex-wrap items-start gap-2 border-b border-border/60 pb-2">
          ...
          <div className="mt-2 flex flex-wrap items-end gap-1.5">
            <label className="flex min-w-0 flex-1 flex-col gap-0.5">
              <span className="text-[10px] text-muted-foreground">테스트용 수위 (m)</span>
              <input
                type="number"
                step="0.01"
                inputMode="decimal"
                ...
                className="h-7 w-full cursor-text rounded border border-border bg-background px-2 text-[11px] tabular-nums text-foreground outline-none placeholder:text-muted-foreground focus:border-primary [color-scheme:light] dark:[color-scheme:dark]"
              />
            </label>
            <button
              type="button"
              ...
              className="inline-flex h-7 cursor-pointer items-center rounded border border-border px-2.5 text-[11px] font-medium text-foreground/90 hover:bg-muted/50"
            >
              적용
            </button>
            {testWaterLevelApplied != null ? (
              <button
                ...
                className="inline-flex h-7 cursor-pointer items-center rounded border border-border px-2 text-[11px] text-muted-foreground hover:bg-muted/50"
              >
                지우기
              </button>
            ) : null}
          ...
          <p className="mt-2 text-[13px] font-semibold tabular-nums text-foreground">
            예상 <span className="text-primary">{riskAreas.length}</span> 건
            <span className="ml-1 text-[11px] font-normal text-muted-foreground">(필지)</span>
          </p>
          {riskError ? (
            ...
            {riskAreas.map((r) => (
              <li key={r.id}>
                <button
                  ...
                  title={`${r.name}로 이동`}
                  onClick={() => flyToRisk(r)}
                  className="w-full cursor-pointer rounded border border-border/60 bg-muted/40 px-2 py-1.5 text-left text-[11px] transition-colors hover:border-sky-200 hover:bg-sky-50/80"
                >
