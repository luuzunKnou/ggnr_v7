...
export function BuildPublicLandDetailPanel({
  detailId,
  onClose,
  onSaved,
  onCreated,
  ...
  const preset = LAYER_ROW_EDIT_PRESETS.publicLand;
  ...
  const { navigateToParcel, movingParcelIdx } = useLayerParcelNavigation(BUILD_PUBLIC_LAND_WMS_LAYER_ID);
  ...
  const loadDetail = useCallback(async () => {
    ...
    try {
      const res = await call("", "POST", {
        ...
        action: "getPublicLandDetailById",
        params: { id },
      });
  ...
  const {
    ...
    wmsLayerId: BUILD_PUBLIC_LAND_WMS_LAYER_ID,
  });
