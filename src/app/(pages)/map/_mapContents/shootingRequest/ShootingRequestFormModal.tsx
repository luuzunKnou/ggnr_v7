      ...
      >
        ...
        {!isNew && existing ? (
          ...
          >
            <div className="flex flex-wrap items-center gap-2 text-[10px]">
              <span className="font-medium text-muted-foreground">처리 상태</span>
              <span
                className={cn(
                  'rounded-full bg-background px-2 py-0.5 font-medium ring-1',
              ...
              {existing.decidedAt ? (
                <span className="text-muted-foreground">처리일 {existing.decidedAt}</span>
              ) : null}
            </div>
            {existing.status === 'rejected' ? (
              <div className="mt-2 rounded-md border border-rose-200 bg-background px-2.5 py-2">
        ...
        {!isNew && !existing ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-2 px-4 text-center">
            <p className="text-xs text-muted-foreground">신청을 찾을 수 없습니다.</p>
            <button
              type="button"
