...
export function ShapeEditorAddLayerDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div
        className="flex max-h-[min(480px,80vh)] w-full max-w-md flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl"
        role="dialog"
        aria-labelledby="add-layer-title"
      >
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          <h2 id="add-layer-title" className="text-sm font-semibold text-slate-800">
            레이어 추가
          </h2>
          <button
            type="button"
        ...
        <ul className="min-h-0 flex-1 overflow-y-auto py-1">
          {loading ? (
            ...
            <li className="px-4 py-6 text-center text-sm text-slate-500">추가할 레이어가 없습니다</li>
          ) : (
            filtered.map((layer) => (
              <li key={layer.id}>
                <button
                  ...
                  onClick={() => onAdd(layer)}
                  className={cn(
                    'flex w-full flex-col px-4 py-2.5 text-left hover:bg-slate-50',
