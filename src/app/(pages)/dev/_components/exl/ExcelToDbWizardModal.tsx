...
export function ExcelToDbWizardModal({ open, onOpenChange, folderName, fileName, fileRelPath, onSuccess }: Props) {
  ...
  const [createSeparateMulgunjiTable, setCreateSeparateMulgunjiTable] = useState(false);
  ...
  /** Excel to DB: 모달 열리면 파싱. 파일명이 레이어 설정과 같으면 그룹·영문·한글명 기본값. 아니면 영문명=폴더명, 한글명=파일명. */
  const initFileRef = useRef<string | null>(null);
  useEffect(() => {
    if (!open) {
      initFileRef.current = null;
      return;
    }
    if (initFileRef.current === fileRelPath) return;
    initFileRef.current = fileRelPath;
    const stem = excelUploadFileStem(fileName);
    ...
    void (async () => {
      try {
        const [lookupRes, downloadRes] = await Promise.all([
          call('', 'POST', {
            service: 'excelUploadService',
        ...
        if (!downloadRes.ok) throw new Error('파일을 불러오지 못했습니다.');
        const blob = await downloadRes.blob();
        const file = new File([blob], fileName, { type: blob.type });
        await handleFileSelected(file);
      } catch (e: unknown) {
        setStep1Warnings([e instanceof Error ? e.message : String(e)]);
      }
    })();
  }, [open, fileRelPath, fileName, folderName, handleFileSelected]);

  /** Excel to DB: 동일 테이블 존재 여부 자동 확인 (버튼 없음) */
  useEffect(() => {
    if (!open) return;
    if (!tableEngOk) return;
    if (layerTableMeta !== null) return;
    void runLayerTableCheck();
  }, [open, tableEng, tableEngOk, layerTableMeta, runLayerTableCheck]);

  /** 별도 물건지 테이블 생성 기본값: 점용대장에서만 체크 */
  useEffect(() => {
    setCreateSeparateMulgunjiTable(excelUploadWorkflowId === 'occupancy_ledger');
  }, [excelUploadWorkflowId]);
  ...
  const runStep4 = useCallback(async () => {
    ...
    const resolveLogPath = (effPath?: string | null): string | null => {
      const p = (effPath ?? pathOrResult ?? fileRelPath ?? '').trim();
      return p || null;
    };
    const flushLogToFile = async (effPath: string | null | undefined) => {
      const p = resolveLogPath(effPath);
      if (!p) {
        pushLog('(경고) 로그 파일 경로를 확인할 수 없어 디스크에 기록하지 못했습니다.');
        return;
      }
      try {
        await call('', 'POST', {
  ...
  return (
    ...
    >
      <DialogContent
        className="w-[1200px] h-[770px] min-w-[1200px] max-w-[95vw] max-h-[90vh] overflow-hidden flex flex-col gap-y-2 p-4"
      ...
      >
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Excel to DB - {stepLabels[step] ?? step} ({step}/{TOTAL_STEPS})
          </DialogTitle>
        </DialogHeader>
              ...
              <div className="rounded-md border border-gray-200 bg-muted/30 p-3 space-y-2">
                ...
                <div className="flex items-center gap-3 flex-wrap text-sm">
                  ...
                  {selectedFileInfo && !step1Validating && (
                    <span className="text-muted-foreground">· {selectedFileInfo.size.toLocaleString()} bytes</span>
                  )}
                </div>
                ...
                <div className="flex flex-wrap items-end gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Input
                      ...
                      className="h-9 w-65 text-sm !text-gray-600"
                    ...
                    <thead>
                      <tr className="bg-gray-100 border-b border-gray-200 sticky top-0 z-[1]">
                        ...
                        {!isLedgerWorkflow && !isAndongRoadUseWorkflow ? (
                          <th
                            className={cn(
                              'text-center font-medium py-0.5 px-1 w-11 border-l border-transparent',
