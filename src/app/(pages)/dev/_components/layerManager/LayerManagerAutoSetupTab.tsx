...
export function LayerManagerAutoSetupTab({ onIssueCountChange }: LayerManagerAutoSetupTabProps) {
  ...
  const fixAll = useCallback(async () => {
    ...
    try {
      ...
      setSuccessMsg(`전체 자동 수정 완료 (성공 ${ok}건${fail > 0 ? `, 실패 ${fail}건` : ""})`)
      await loadIssues()
    } catch (e) {
  ...
  }, [filteredLayers, loadIssues])

  if (loading) return <p className="text-sm text-muted-foreground p-2">스캔 중...</p>
    ...
    <div className="flex flex-col gap-3 p-2 min-h-0 h-full">
      ...
      {filteredLayers.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          {layers.length === 0 ? "설정 오류가 있는 레이어가 없습니다." : "표시할 레이어가 없습니다."}
        </p>
      ) : (
