...
const jijukParcelSelectSql = (geomCol: string, jijukSchema: string) => `
  SELECT
    j.pnu::text AS pnu,
...
export async function resolveJijukParcelGeomsByAddresses(params: {
  ...
  const jijukSchema = await resolveJijukSchema();
  const jijukOrderCol = await resolveJijukOrderColumn(jijukSchema);
  ...
  for (const item of items) {
    ...
    const selectSql = jijukParcelSelectSql('j.geom', jijukSchema);
    ...
    try {
      ...
      if (hasPoint) {
        ...
        mapped = await runQuery(`ST_Intersects(${jijukGeom5181Sql('j.geom')}, ${point5181})`, `j.${jijukOrderCol}`);
      }
      ...
      if (!mapped?.geometry3857) {
        ...
        if (pnuWhere) {
          mapped = await runQuery(pnuWhere, `j.${jijukOrderCol}`);
        }
      }
...
export async function listJijukParcelsByGeomWkt5181(params: {
  ...
  const queryStr = `
    ${jijukParcelSelectSql('j.geom', jijukSchema)}
    ...
    ORDER BY j.${quoteIdent(jijukOrderCol)}
    LIMIT ${limit}`;
...
export async function subtractParcelFromParentWkt5181(params: {
  ...
  if (geoJson != null && typeof geoJson === 'object') {
    ...
    if (pnuWhere) {
      const jijukSchema = await resolveJijukSchema();
      const jijukOrderCol = await resolveJijukOrderColumn(jijukSchema);
      subtractExpr = `(SELECT ${jijukGeom5181Sql('j.geom')} FROM ${quoteIdent(jijukSchema)}.${quoteIdent('jijuk')} j WHERE j.geom IS NOT NULL AND ${pnuWhere} ORDER BY j.${quoteIdent(jijukOrderCol)} LIMIT 1)`;
    }
  }
  if (!subtractExpr) {
...
export async function syncChildParcelsByParentId(params: {
  ...
  try {
    ...
    const jijukSchema = geomCol ? await resolveJijukSchema() : null;
    const jijukOrderCol = jijukSchema ? await resolveJijukOrderColumn(jijukSchema) : null;
    ...
    for (const row of parcelRows) {
      ...
      const pnuWhere = pnuDigits ? buildJijukPnuMatchWhereSql(pnuDigits) : null;
      const geomInsert =
        geomCol && pnuWhere && jijukSchema && jijukOrderCol
          ? `(SELECT j.geom FROM ${quoteIdent(jijukSchema)}.${quoteIdent('jijuk')} j
