...
$featureservExe = Join-Path $featureservDir "pg_featureserv.exe"
$configFile = Join-Path $servicesDir "pg_featureserv.toml"
...
if (-not (Test-Path $featureservExe)) {
    Write-Host "pg_featureserv가 설치되지 않았습니다. 수동으로 설치해주세요." -ForegroundColor Red
    exit 1
}

Write-Host "pg_featureserv를 시작합니다..." -ForegroundColor Green
Write-Host "서비스 URL: http://192.168.120.82:9000" -ForegroundColor Cyan

if (Test-Path $configFile) {
    & $featureservExe --config $configFile
} else {
    & $featureservExe
}
