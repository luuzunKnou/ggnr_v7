...
for /f "usebackq tokens=1,* delims==" %%A in ("%DEMO_ENV_FILE%") do (
  ...
  if /i "%%A"=="GGNR_DATA_UNC_ROOT" set "DEMO_GDRIVE_UNC=%%B"
)
...
if not defined DEMO_GDRIVE_UNC (
  echo [ERROR] GGNR_DATA_UNC_ROOT not in credential file.
  set "FAIL_EC=1"
  exit /b 1
)
echo [OK] demo G: ready. account=!DEMO_GDRIVE_USER! object=!DEMO_GDRIVE_OBJECT!
echo      UNC=!DEMO_GDRIVE_UNC!
exit /b 0
...
echo :: Generated by 00_make_ggnr_starter.bat — do not hand-edit redirect syntax
echo.
echo :: encoding
echo chcp 65001 ^> nul
echo title GGNR_V7 start
echo.
...
echo   echo [ERROR] cd failed: %ROOT%
echo   goto :fail_pause
echo ^)
echo if not exist "package.json" ^(
echo   echo [ERROR] package.json missing in %%CD%%
echo   goto :fail_pause
echo ^)
echo.
)
:: demo: map G: (must be "2>&1" not bare "2>" — bare form aborts CMD instantly)
if /i "%ENV_NAME%"=="demo" if defined DEMO_GDRIVE_UNC (
  >> "%OUT%" (
  echo :: G: = %DEMO_GDRIVE_UNC% — map for interactive/session; fail is non-fatal
  echo net use G: %DEMO_GDRIVE_UNC% /persistent:yes ^>nul 2^>^&1
  echo if errorlevel 1 echo [WARN] net use G: failed - app uses GGNR_DATA_UNC_ROOT if set
  echo.
  )
)
>> "%OUT%" (
...
echo :start_fail
echo echo.
echo echo [ERROR] start failed. Check messages above and C:\logs\GGNR_V7_stderr.log
echo goto :fail_pause
echo.
echo :fail_pause
echo echo.
echo if /i "%%GGNR_START_NO_PAUSE%%"=="1" ^(
echo   echo [INFO] GGNR_START_NO_PAUSE=1 - skip pause ^(nssm^)
echo   exit /b 1
echo ^)
echo echo -----------------------------------------------------------
